    ...
    <section className="relative max-w-[1400px] mx-auto px-3 sm:px-6 mt-[15px]">
      <div className="relative overflow-hidden rounded-[20px]">
          ...
          <div className="embla__container">
            {slides.map((s, i) => (
              <div
                ...
                className="embla__slide relative min-w-0 w-full aspect-[16/6] overflow-hidden">
                <img
                  src={s.src}
                  alt=""
                  loading={i === 0 ? "eager" : "lazy"}
                  decoding="async"
                  className="absolute inset-0 w-full h-full object-cover scale-110 blur-2xl opacity-70k"
                />
                <img
                  src={s.src}
                  alt=""
        ...
        <button
          onClick={scrollPrev}
          aria-label="Previous"
          className="hidden sm:flex absolute left-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full
             bg-black/45 hover:bg-black/70 backdrop-blur-md text-white
        ...
        <button
          onClick={scrollNext}
          aria-label="Next"
          className="hidden sm:flex absolute right-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full
             bg-black/45 hover:bg-black/70 backdrop-blur-md text-white
             ring-1 ring-white/25 shadow-lg
             items-center justify-center transition
             focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
